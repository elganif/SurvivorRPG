...
class QuadTree
{
    ...
    int getFoes(olc::vf2d targetLoc, float range, int numTarg, TARG targType, std::list<std::shared_ptr<Entity>>& returns);
    int getFoes(olc::vf2d targetLoc, float range, int numTarg, std::function<bool(const std::shared_ptr<Entity> f, const std::shared_ptr<Entity> s)> targType, std::list<std::shared_ptr<Entity>>& returns,float curDist2);
